...
After editing Python files (like `llm_jupyter_extension/routes.py`):

```bash
python -m py_compile llm_jupyter_extension/__init__.py  # Check single file for syntax errors
```
...
**Python** (in `llm_jupyter_extension/*.py` files):
...
- Backend processing in `llm_jupyter_extension/routes.py`
...
- **✅ Do**: `llm_jupyter_extension/` with underscores, all lowercase
...
```typescript
const PLUGIN_ID = 'llm_jupyter_extension:plugin';
```
...
export namespace CommandIDs {
  export const openPanel = 'llm_jupyter_extension:open-panel';
  export const refreshData = 'llm_jupyter_extension:refresh-data';
}
...
const plugin: JupyterFrontEndPlugin<void> = {
  id: 'llm_jupyter_extension:plugin',
  autoStart: true,
...
- Backend routes: `llm_jupyter_extension/routes.py` (class `DataRouteHandler`)
- Frontend logic: `src/` directory
- Python package: `llm_jupyter_extension/` directory
...
1. **Read the backend first** — Check `llm_jupyter_extension/routes.py` to understand the existing API contract
...
Create RESTful endpoints in `llm_jupyter_extension/routes.py`:
...
def setup_route_handlers(web_app):
    ...
    data_route = url_path_join(base_url, "llm_jupyter_extension", "data")
...
- Define matching endpoint path strings (e.g., `"hello"`, `"get-data"`) in both `llm_jupyter_extension/routes.py` and `src/api.ts` to ensure routes sync between backend and frontend
...
2. **Write the code** (TypeScript in `src/`, styles in `style/`, Python in `llm_jupyter_extension/`)
...
5. **Install the extension** (REQUIRED for JupyterLab to recognize it):
   ...
   jupyter server extension enable llm_jupyter_extension
   ```
...
- **Output**: Creates files in `lib/` and `llm_jupyter_extension/labextension/`
...
**After editing Python** (files in `llm_jupyter_extension/`):
...
- Ask user to search for the extension ID (`llm_jupyter_extension`) to see if it loaded
...
ls -la llm_jupyter_extension/labextension/  # Should contain bundled extension
```
...
- Backend routes: `llm_jupyter_extension/routes.py`
- Server extension config: `jupyter-config/server-config/llm_jupyter_extension.json`
...
**❌ Don't**: Write frontend interfaces without first checking the backend API contract in `llm_jupyter_extension/routes.py`
...
```css
.jp-llm-jupyter-extension-widget {
  padding: 8px;
}
```
...
**❌ Don't**: Use absolute imports like `from llm_jupyter_extension.routes import ...`
...
- **Plugin ID** (in `src/index.ts`): `'llm_jupyter_extension:plugin'`
- **Command IDs** (in `src/commands.ts` or `src/index.ts`): `'llm_jupyter_extension:command-name'`
...
- **CSS classes** (in `style/index.css`): `.jp-llm-jupyter-extension-ClassName`
- **API routes** (in `llm_jupyter_extension/routes.py`): `url_path_join(base_url, "llm_jupyter_extension", "endpoint")`
